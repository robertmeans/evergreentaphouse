...
function print_ajde_customization_form($cutomization_pg_array, $ajdePT, $extra_tabs=''){
	...
	foreach($cutomization_pg_array as $cpa=>$cpav){								
		...
		$leftside .= "<li ".$ls_level_code."><a class='".( ($count==1)?'focused':null)."' data-c_id='".$cpav['id']."' title='".$cpav['tab_name']."'><i class='fa fa-".( !empty($cpav['icon'])? $cpav['icon']:'edit')."'></i>".__($cpav['tab_name'],$ajde->domain)."</a></li>";								
		...
		if( $tab_type !='empty'){ // to not show the right side
			...
			$rightside.= "<div id='".$cpav['id']."' style='".$display_default."' class='nfer'>
				<h3>".__($cpav['name'],$ajde->domain)."</h3>";
			...
			foreach($cpav['fields'] as $field){
				...
				// LEGEND or tooltip
				$legend_code = (!empty($field['legend']) )? $ajde->wp_admin->tooltips($field['legend'], 'L', false):
					( (!empty($field['tooltip']) )? $ajde->wp_admin->tooltips($field['tooltip'], 'L', false): null );
				
				
				switch ($field['type']){
					// notices
					case 'notice':
						$rightside.= "<div class='ajdes_notice'>".__($field['name'],$ajde->domain)."</div>";
					break;
					//IMAGE
					case 'image':
						$image = ''; 
						$meta = $ajdePT[$field['id']];
					...
					case 'subheader':
						$rightside.= "<h4 class='acus_subheader'>".__($field['name'],$ajde->domain)."</h4>";
					break;
					case 'note':
						$rightside.= "<p class='nylon_note'><i>".__($field['name'],$ajde->domain)."</i></p>";
					break;
					...
					case 'text':
						...
						$rightside.= "<p>".__($field['name'],$ajde->domain).$legend_code."</p><p><span class='nfe_f_width'><input type='text' name='".$field['id']."'";
						$rightside.= 'value="'.$FIELDVALUE.'"';
						$rightside.= $default_value."/></span></p>";
					break;
					case 'textarea':						
						$rightside.= "<p>".__($field['name'],$ajde->domain).$legend_code."</p><p><span class='nfe_f_width'><textarea name='".$field['id']."'>".$FIELDVALUE."</textarea></span></p>";
					break;
					case 'font_size':
						$rightside.= "<p>".__($field['name'],$ajde->domain)." <select name='".$field['id']."'>";
					...
					case 'radio':
						$rightside.= "<p class='acus_line acus_radio'>".__($field['name'],$ajde->domain)."</br>";
						$cnt =0;
						foreach($field['options'] as $option=>$option_val){
							...
							.  $checked_or_not  ."/><label class='ajdebe_radio_btn' for='".$option_id."'><span class='fa'></span>".__($option_val,$ajde->domain)."</label></em>";
					...
					case 'dropdown':
						...
						$rightside.= "<p class='acus_line {$field['id']}'>".__($field['name'],$ajde->domain)." <select class='ajdebe_dropdown' name='".$field['id']."'>";
					...
					case 'checkboxes':
						...
						echo "<p class='acus_line acus_checks'><span style='padding-bottom:10px;'>".__($field['name'],$ajde->domain)."</span>";
					...
					case 'rearrange':
						...
						ob_start();
							...
							$SAVED_ORDER = (!empty($ajdePT[$_ORDERVAR]))?
								array_filter(explode(',', $ajdePT[$_ORDERVAR])): false;
							...
							echo '<input class="ajderearrange_order" name="'.$_ORDERVAR.'" value="'.( (!empty($ajdePT[$_ORDERVAR]))? $ajdePT[$_ORDERVAR]:null).'" type="hidden"/>
					...
					case 'yesno':						
						...
						$rightside.= "<p class='yesno_row'>".$ajde->wp_admin->html_yesnobtn(array('var'=>$__default,'attr'=>array('afterstatement'=>$after_statement) ))."<input type='hidden' name='".$field['id']."' value='".(($__default=='yes')?'yes':'no')."'/><span class='field_name'>".__($field['name'],$ajde->domain)."{$legend_code}</span>";
	...
	<table id='ajde_customization'>
		<tr><td class='backender_left' valign='top'>
			...
			<div class="ajde-collapse-menu"><div id="collapse-button" class='ajde_collpase_btn'><div></div></div><span><?php _e('Collpase Menu',$ajde->domain);?></span></div>
