		...
		public function eventon_generate_events($args){
			...
			if(!empty($ecv['sep_month']) && $ecv['sep_month']=='yes' && $ecv['number_of_months']>1 ){
				...
				$months_event_array = $this->generate_event_data( 
					$event_list_array, 	$ecv['focus_start_date_range']
				);
					...
					// PAYPAL Code
						if(!empty($ev_vals['evcal_paypal_item_price'][0]) && $this->evopt1['evcal_paypal_pay']=='yes' 
							...
							$_eventcard['paypal'] = array(
								'title'=>$event->post_title .' ('.date('Y-m-d h:ia',$event_start_unix) .')',
					...
					// Event Organizer
						...
						{							
							$_eventcard['organizer'] = array(
								...
								'exlink'=>(!empty($ev_vals['evcal_org_exlink'])? $ev_vals['evcal_org_exlink'][0]:false),
					...
					// LEARN MORE and ICS
						if(!empty($ev_vals['evcal_lmlink']) || !empty($this->evopt1['evo_ics']) && $this->evopt1['evo_ics']=='yes'){
							$_eventcard['learnmoreICS'] = array(						
								...
								'excerpt'=> ( !empty($event_full_description)? eventon_get_normal_excerpt($event_full_description, 30): $event->post_title ),
								'evals'=>$ev_vals,								
							);
						}
